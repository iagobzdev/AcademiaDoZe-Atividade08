...
public class AlunoInfrastructureTests : TestBase
{
    ...
    {
        _logradouroRepo = new LogradouroRepository(ConnectionString);
        _alunoRepo = new AlunoRepository(ConnectionString);
    }
    ...
    public async Task Aluno_Atualizar_Sucesso()
    {
        ...
        var aluno = await _alunoRepo.Adicionar(Aluno.Criar(
            0, "Iago", GerarCpf(), DateOnly.FromDateTime(DateTime.Today.AddYears(-20)),
            GerarTelefone(), GerarEmail(), logradouro, "100", "Barboza",
            "SenhaValida123SQLite", foto).Value!);
    ...
    {
        var nomeUnico = "AlunoNomeUnico" + Guid.NewGuid().ToString("N")[..8];
        var logradouro = await LogradouroInfrastructureTests.CriarEInserirLogradouroAsync(_logradouroRepo);
        var foto = Arquivo.Criar(new byte[] { 9, 9 }).Value!;
        await _alunoRepo.Adicionar(Aluno.Criar(
            0, nomeUnico, GerarCpf(), DateOnly.FromDateTime(DateTime.Today.AddYears(-22)),
            GerarTelefone(), GerarEmail(), logradouro, "50", "",
            "SenhaValida123", foto).Value!);

        var resultados = await _alunoRepo.ObterPorNome(nomeUnico);
        Assert.NotNull(resultados);
        Assert.Contains(resultados, a => a.Nome == nomeUnico);

        var resultadosVazio = await _alunoRepo.ObterPorNome("NomeCompletamenteInexistente_999");
