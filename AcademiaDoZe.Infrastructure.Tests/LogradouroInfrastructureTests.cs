...
public class LogradouroInfrastructureTests : TestBase
{
    ...
    {
        _repository = new LogradouroRepository(ConnectionString);
    }

    internal static async Task<Logradouro> CriarEInserirLogradouroAsync(LogradouroRepository logradouroRepo)
    {
        var cep = GerarCep();
        var logradouroResult = Logradouro.Criar(0, cep, "Iago", "Barboza", "SQLite", "SC", "Brasil");
    ...
    {
        var cep = GerarCep();
        var logradouro = Logradouro.Criar(0, cep, "Iago", "Barboza", "SQLite", "SC", "Brasil").Value!;
        ...
        Assert.Equal("Iago", inserido.Nome);
        Assert.Equal("Barboza", inserido.Bairro);
        Assert.Equal("SQLite", inserido.Cidade);
    ...
    {
        var cep = GerarCep();
        var cidadeUnica = "CidadeUnica" + Guid.NewGuid().ToString("N")[..5];
        var logradouro = Logradouro.Criar(0, cep, "Rua X", "Bairro Y", cidadeUnica, "SC", "Brasil").Value!;
        await _repository.Adicionar(logradouro);

        // Comparação exata: SQLite usa colação BINARY (case-sensitive) por padrão para "=".
    ...
    {
        var cep = GerarCep();
        var cidade = "Cidade" + Guid.NewGuid().ToString("N")[..5];
        var bairro = "Bairro" + Guid.NewGuid().ToString("N")[..5];
